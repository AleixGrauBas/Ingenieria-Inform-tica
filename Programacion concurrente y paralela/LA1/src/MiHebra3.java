...
public class MiHebra3 extends Thread{
    ...
    public MiHebra3(int miId, int numHebras, Vector<String> linea, ConcurrentHashMap<String,Integer> table){
        this.miId=miId;
        ...
        for (int i = miId; i < lineas.size(); i += numHebras) {
            ...
            for (int j = 0; j < palabras.length; j++) {
                ...
                if (palabraActual.length() > 0) {
                    EjemploPalabraMasUsada.contabilizaPalabra3(table, palabraActual);
                }
            }
